...
绝大多数生成式引擎（ChatGPT 联网模式、Perplexity、Google AI Overviews、Kimi、Doubao）的底层是同一个架构：**RAG（Retrieval-Augmented Generation，检索增强生成）**。关于 RAG 本身的演进、流派与工程权衡，可以参见 [[rag-landscape-2026|RAG 技术全景与流派分析]]。
...
LLM 的 chunking 经常会把你的内容切成 200-500 token 的小块。如果一段必须配合上下文才能理解（比如频繁出现"它""这""上一节提到"），切碎之后那一块就失去信息密度，不会被选中。（Claude Code 之类的工程系统为什么干脆不用 RAG / chunking，我在 [[claude-code-vs-xiaonuan-memory|从 Claude Code 到小暖]] 里展开讨论过。）
...
在 GEO 时代，AI 的偏好恰恰回到了最朴素的好内容定义：**有数据、有出处、有结构、有原创观点、能独立成立的段落。** 这其实就是新闻系一年级教学生写新闻稿的标准——也回到了 [[weishenme-xie|为什么写作]] 这件事本身。
